...
A `title` adds a small label above the value — useful when the value alone needs naming.
...
## Driven by computed data

::: {style='font-size: 80%;'}
`color` and `delta` don't have to be typed in by hand — since a value box is just a fenced div, a code cell can compute them and print the div's markdown straight into the document with `output: asis`. Here Python looks at a week-over-week change, picks red or green depending on its sign, formats the delta string, and plots a matching sparkline — all from the same numbers:
:::
...
ax.plot(weekly_sales, marker="o", color="#2e7d32" if change_pct >= 0 else "#c62828")
ax.axis("off")
plt.tight_layout()
plt.show()
```
...
```{.python}
#| output: asis
print(f'::: {{.value-box title="Weekly sales" value="{current}" delta="{delta}" color="{color}"}}')
print("Colour and delta both follow the sign of `change_pct`, computed above")
print(":::")
```
...
#| output: asis
print(f'::: {{.value-box title="Weekly sales" value="{current}" delta="{delta}" color="{color}"}}')
print("Colour and delta both follow the sign of 'change_pct', computed on the previous slide")
print(":::")
```
...
::: {style='font-size: 75%;'}
`output: asis` tells Quarto to drop the cell's printed text straight into the document as markdown, so the `f-string`-built `::: {.value-box ...}` line above is parsed exactly as if it had been typed by hand. Flip `weekly_sales[-1]` below `weekly_sales[-2]` and both the box and the chart switch to red together, since they're reading the same `change_pct`.
:::
