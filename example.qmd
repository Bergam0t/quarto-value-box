...
Quarto compiles a project's `_brand.yml` palette into CSS custom properties automatically, so `color` can reference it with `var(...)` too — no manual `<style>` block needed. On `revealjs`, `color.primary` becomes `--r-link-color` (already live on this deck's own theme, with no `_brand.yml` added here):

::: {.value-box-row columns="2"}

::: {.value-box color="var(--r-link-color)" height="100px"}
color="var(--r-link-color)"
:::

::: {.value-box color="bg-grey" icon="bi-star" icon-color="var(--r-link-color)" height="100px"}
